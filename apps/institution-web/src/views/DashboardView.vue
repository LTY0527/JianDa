...
import { Upload, ArrowRight, FileClock, CircleCheck, BookOpen, TriangleAlert } from "lucide-vue-next";
  ...
  <div>
    ...
    <div class="dashboard-grid">
      <section class="panel">
        ...
        <table v-if="recent.length"><thead><tr><th>材料名称</th><th>状态</th><th>更新时间</th><th></th></tr></thead><tbody><tr v-for="row in recent" :key="row.id"><td><b>{{ row.title }}</b><small>{{ row.file_name || "公开信息导入" }}</small></td><td><StatusTag :status="row.status" :text="statusText[row.status] || row.status" /></td><td>{{ row.updated_at }}</td><td><RouterLink :to="row.status === 'WAITING_REVIEW' ? `/documents/${row.id}/review` : `/documents/${row.id}/process`">查看详情</RouterLink></td></tr></tbody></table>
      ...
      <aside class="panel todo">
        ...
        <ol v-if="waiting.length"><li v-for="(row,index) in waiting.slice(0,4)" :key="row.id"><span>{{ index + 1 }}</span><div><b>{{ row.title }}</b><small>结构化结果已生成，等待人工对照审核</small></div></li></ol>
