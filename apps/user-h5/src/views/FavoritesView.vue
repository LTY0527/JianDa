...
async function load() { try { available.value = await fetchItems(undefined, activeRegion.value.region_code); } catch { available.value = []; } }
onMounted(() => { window.addEventListener("jianda-library-change", refresh); void load(); });
