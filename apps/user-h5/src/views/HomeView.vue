...
const newsStream = computed(() => todayReads.value.slice(1));
...
<template><div class="h5-page"><H5Header /><main class="h5-main home-main">
  ...
  <nav class="categories" aria-label="快捷频道"><RouterLink v-for="c in cats" :key="c[0]" :to="`/category/${c[2]}`"><span><component :is="c[1]" /></span><b>{{ c[0] }}</b></RouterLink><RouterLink class="all-channel" to="/news"><span><ChevronRight /></span><b>全部资讯</b></RouterLink></nav>
  ...
  <template v-else>
    <section v-if="featured" class="featured-story">
      ...
      <div><span>{{featured.category}}{{featured.is_local?" · 本地":""}}</span><h2>{{featured.title}}</h2><p>{{featured.summary}}</p><small>{{featured.source_name}} · {{String(featured.published_at).slice(0,10)}} · {{featured.reading_minutes||1}}分钟阅读</small><RouterLink :to="`/news/${featured.slug}`">查看适老版<ArrowRight/></RouterLink></div>
    </section>
