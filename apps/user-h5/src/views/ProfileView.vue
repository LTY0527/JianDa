...
const profile = ref<ResidentProfile|null>(null); const username = ref("demo_chen"); const password = ref("Resident@123"); const loginError = ref(""); const loggingIn = ref(false);
...
async function login(){ loggingIn.value=true;loginError.value="";try{ profile.value=await residentLogin(username.value,password.value); }catch{ loginError.value="账号或密码不正确，请使用页面标注的 DEMO 账号。"; }finally{ loggingIn.value=false; } }
