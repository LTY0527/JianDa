...
const links = [
  ["/reminders", Bell, "我的提醒", "报名截止、活动和办理时间", () => ""],
...
<template><div class="h5-page"><H5Header /><main class="h5-main profile-page"><section class="guest-card"><span><UserRound /></span><div><h1>游客使用</h1><p>收藏、历史、提醒和偏好与当前设备游客标识关联，不作为身份认证凭证。</p></div></section><section class="profile-links"><RouterLink v-for="link in links" :key="link[2]" :to="link[0]"><component :is="link[1]" /><span><b>{{ link[2] }}</b><small>{{ link[3] }}</small></span><em>{{ link[4]() }}</em><ChevronRight /></RouterLink></section><section class="profile-links secondary"><div><CircleHelp /><span><b>帮助与反馈</b><small>查看使用说明和反馈渠道</small></span></div><div><ShieldCheck /><span><b>隐私说明</b><small>不使用浏览器指纹、广告 ID 或精确定位</small></span></div><div><Info /><span><b>关于简达</b><small>人工审核的适老化公共服务平台</small></span></div></section><button class="clear-local" type="button" @click="clear"><Trash2 />清除本机收藏和历史</button></main><BottomNav /></div></template>
