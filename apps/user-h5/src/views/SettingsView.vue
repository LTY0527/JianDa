...
import { Type, Volume2, Contrast, Play, LayoutList, MoveHorizontal } from "lucide-vue-next";
const size = ref(Number(localStorage.getItem("jianda_font") || 18));
const rate = ref(Number(localStorage.getItem("jianda_rate") || 1));
...
<section><header><Volume2 /><div><h2>语音速度</h2><p>选择适合您的朗读速度</p></div></header><div class="rate-options"><button v-for="r in [0.8,1,1.2]" :key="r" :class="{active:rate===r}" @click="rate=r;save()">{{ r===0.8?'较慢':r===1?'适中':'较快' }}</button></div></section>
<section class="setting-switches"><header><Contrast /><div><h2>高对比度</h2><p>加深文字和边框</p></div><label class="switch"><input type="checkbox" v-model="contrast" @change="save" /><i></i></label></header><header><Play /><div><h2>打开详情后自动朗读</h2><p>仅在浏览器支持语音时生效</p></div><label class="switch"><input type="checkbox" v-model="preferences.autoRead" @change="save" /><i></i></label></header><header><LayoutList /><div><h2>首页显示最近浏览</h2><p>关闭后历史仍保存在“我的”</p></div><label class="switch"><input type="checkbox" v-model="preferences.showRecent" @change="save" /><i></i></label></header><header><MoveHorizontal /><div><h2>桌面左右切换文章</h2><p>使用阅读区两侧按钮或键盘方向键</p></div><label class="switch"><input type="checkbox" v-model="preferences.desktopSideNavigation" @change="save" /><i></i></label></header><header><MoveHorizontal /><div><h2>手机左右滑动切换</h2><p>左滑下一篇，右滑上一篇</p></div><label class="switch"><input type="checkbox" v-model="preferences.mobileSwipeNavigation" @change="save" /><i></i></label></header><header><Volume2 /><div><h2>切换文章时停止朗读</h2><p>避免上一篇语音继续播放</p></div><label class="switch"><input type="checkbox" v-model="preferences.stopSpeechOnNavigation" @change="save" /><i></i></label></header><header><LayoutList /><div><h2>优先阅读同分类文章</h2><p>上一篇和下一篇优先保持当前分类</p></div><label class="switch"><input type="checkbox" v-model="preferences.preferSameCategory" @change="save" /><i></i></label></header></section>
