...
function openImagePicker(){mediaInput.value?.click();}
...
<template><div class="h5-page"><H5Header/><main class="h5-main neighborhood-page">
  ...
  <section v-if="loggedIn" class="neighborhood-compose"><label for="post-content">分享一件对邻里有用的事</label><textarea id="post-content" v-model="draft" maxlength="500" rows="3" placeholder="不发布身份证、银行卡、门牌等个人信息"></textarea><div v-if="selected.length" class="post-media-preview"><figure v-for="(item,index) in selected" :key="item.preview"><img :src="item.preview" :alt="`待发布图片 ${index+1}`"/><button type="button" :aria-label="`移除图片 ${index+1}`" @click="removeImage(index)"><Trash2/></button></figure></div><footer><span>{{ draft.length }}/500 · {{ selected.length }}/6 张</span><input ref="mediaInput" class="community-media-input" type="file" accept="image/jpeg,image/png" multiple tabindex="-1" aria-hidden="true" @change="choose"/><button type="button" class="image-picker" @click="openImagePicker"><ImagePlus/>选择图片</button><button type="button" class="community-publish-button" :disabled="submitting || uploading || !draft.trim()" @click.stop.prevent="publish"><Send/>{{ uploading?'上传图片…':submitting ? "发布中…" : "发布" }}</button></footer></section>
