...
import AmapRegionMap from "./AmapRegionMap.vue";
  ...
  <Teleport to="body">
    <div v-if="regionOpen" class="region-dialog" role="presentation" @click.self="regionOpen = false">
      <section role="dialog" aria-modal="true" aria-labelledby="region-dialog-title">
        ...
        <nav class="region-breadcrumb" aria-label="地区层级">中国 <ChevronRight /> 上海市 <ChevronRight /> 宝山区 <ChevronRight /> {{ activeRegion.street_or_town }}</nav>
        <AmapRegionMap :regions="supportedRegions" :active-code="activeRegion.region_code" @select="chooseRegion" />
