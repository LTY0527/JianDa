...
11. 表格或句子明确给出“日期、时间、地点”对应关系时，同时输出 sessions；每个 session 必须引用包含三者的同一连续原文。
12. 不得按 EVENT_DATE 和 SERVICE_TIME 数组下标猜测场次；无法确认对应关系时不创建猜测场次，或将 needs_human_review 标记为 true。
13. TARGET_AUDIENCE 只保留人群和年龄范围；发热、禁忌、资格限制放入 ELIGIBILITY，二者不得返回完全相同的 value。
