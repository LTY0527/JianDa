...
采集产物始终先成为待审核材料，不会自动审核或自动发布。正文变化会创建新版本，线上已发布版本继续公开；新版本完成文字、来源和图片候选审核后才能替换。第三方图片候选记录 URL、来源页、发现方式、alt、尺寸、MIME、hash、缓存和权利状态；未填写来源与许可说明不能确认，拒绝或无合适候选时使用本地分类默认图。
