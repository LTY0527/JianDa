...
1. 使用 `platform_admin` 进入“公开信息导入”，在“导入网页文章”中填写已登记的官方文章 URL。
2. 先查看预览中的来源、robots 状态、发布时间、正文摘要、封面类型和图片许可提示；预览不会创建材料。
3. 确认导入后发起 AI 处理，按内容类型核对适老摘要、事实引用和官方原文链接。
...
- 网页 Collector 当前只支持人工 URL 预览和确认导入，不执行无限制自动爬取；非白名单域名、robots 禁止页面和受限内容不会采集。
