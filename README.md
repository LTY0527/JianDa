...
用户端固定五项一级导航：`/` 首页、`/news` 资讯、`/assistant` 简达助手、`/services` 办事、`/profile` 我的；一级页面不显示返回按钮，详情、收藏、历史、设置和助手历史等二级页面使用带 fallback 的安全返回。

1. 在首页查看重要提醒、今日必看、办事快报和权威资讯，进入资讯频道体验频道、重要程度、收藏和搜索筛选。
2. 在办事专区按服务对象、事项类型和发布机构筛选，进入详情查看材料、步骤、来源和原文。
3. 收藏一条内容后到“我的 → 我的收藏”查看；打开详情会自动写入“历史浏览”。
4. 在阅读设置切换 18/20/22/24px 字号、语速、高对比度、自动朗读、关注频道和首页最近浏览。
5. 从底部中央入口进入“简达助手”，选择推荐问题或输入文字；也可以从办事详情点击“问问这个事项”携带当前内容上下文。
6. 助手回答会展示来源机构、发布日期、原文片段及详情链接；历史会话可从输入区上方进入。

收藏快照、浏览历史、阅读偏好和助手会话保存在当前浏览器 `localStorage`，游客不需要登录。收藏页会重新与公开列表求交集，因此已撤回内容不会继续显示。清除浏览器站点数据或在页面内执行清除操作会删除相应本机记录。
